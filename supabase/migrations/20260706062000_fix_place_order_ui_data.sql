-- ============================================================
-- Migration: Fix Place Order UI Data
-- - Populates the denormalized `orders.items` JSONB column
-- - Adds `total_price` and frontend-specific keys to fix UI rendering
-- 
-- TECH DEBT FLAG: Dual-Source of Truth
-- The database currently has two sources of truth for order items:
-- 1. The normalized `order_items` table (used for backend reporting)
-- 2. The denormalized `orders.items` JSONB column (read by the frontend)
-- There are NO database triggers syncing these. Any future flow (e.g. admin 
-- order editing, partial returns) MUST remember to update BOTH.
-- PROPER REFACTOR PLAN: Update frontend hooks (e.g. `useOrders.ts`) to use 
-- a `select: '*, order_items(*)'` join, and update `normalizeModernOrder` 
-- to read `record.order_items`, completely deprecating the JSONB column.
-- ============================================================
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Cache item data for order_items insert AND frontend JSONB compatibility
    v_item_rows := v_item_rows || jsonb_build_object(
      ...
      -- Frontend fallback expectations
      'product_name', v_db_name,
      'product_image', v_db_image,
      'unit_price', v_db_price,
      'total_price', (v_db_price * v_quantity)
    );
  END LOOP;
  ...
  -- Calculate remaining payment required
  IF v_total - v_wallet_deducted > 0 AND p_payment_method = 'wallet' THEN
    RAISE EXCEPTION 'Insufficient wallet balance to cover the order total';
  END IF;
  ...
  INSERT INTO public.orders (
    ...
    discount_amount,
    item_count,
    items -- [NEW] Populating JSONB frontend cache
  ) VALUES (
    v_order_id,
    v_order_id,
    v_actor_id,
    ...
    v_coupon_discount,
    v_wallet_deducted,
    ROUND(v_subtotal)::INTEGER,
    ROUND(v_tax)::INTEGER,
    0,
    ROUND(v_total)::INTEGER,
    ROUND(v_coupon_discount)::INTEGER,
    v_item_count,
    v_item_rows -- [NEW] Populating JSONB frontend cache
  )
  ...
  FOR v_item_rec IN SELECT * FROM jsonb_array_elements(v_item_rows) LOOP
    INSERT INTO public.order_items (
      ...
      ROUND((v_item_rec.value->>'price')::NUMERIC)::INTEGER,
      ROUND(((v_item_rec.value->>'price')::NUMERIC * (v_item_rec.value->>'quantity')::INTEGER))::INTEGER,
      jsonb_build_object('size', v_item_rec.value->>'size', 'color', v_item_rec.value->>'color')
    );
  END LOOP;

  -- ── Phase 7: Clear user cart ──────────────────────────────
  ...
  RETURN json_build_object(
    ...
    'remaining_amount', v_total - v_wallet_deducted
  );

EXCEPTION
  WHEN OTHERS THEN
    -- Any exception rolls back the entire transaction automatically
    RETURN json_build_object(
      'success', false,
      'error', SQLERRM
    );
