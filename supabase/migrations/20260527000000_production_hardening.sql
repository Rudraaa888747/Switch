-- ============================================================
-- Migration: Production Hardening — Stock Race Condition Fix
-- + Coupon is_active validation
-- + Admin notification from server-side
-- ============================================================

-- ────────────────────────────────────────────────────────────
-- 0. SCHEMA UPDATES
--    Ensure required columns for modern checkout exist
-- ────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- We use a conditional UPDATE (WHERE stock >= qty) + check FOUND.
  -- This is safe under concurrent load — no separate read step.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- only one will satisfy stock_quantity >= v_quantity after the lock.
  ...
  IF p_coupon_code IS NOT NULL AND p_coupon_code <> '' THEN
    ...
    WHERE code = upper(trim(p_coupon_code))
      AND is_active = true                            -- ← was missing before
  ...
  -- Apply coupon discount proportionally to the tax
...
-- ── Grant execute (authenticated users may call this to place orders) ──
REVOKE ALL ON FUNCTION public.place_order_secure FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.place_order_secure TO authenticated, service_role;

-- ────────────────────────────────────────────────────────────
-- 2. REVOKE OLD/INSECURE ORDER RPCS
--    place_order_with_wallet is superseded by place_order_secure.
--    Remove EXECUTE grant from authenticated so it can no longer
--    be called from the client.
-- ────────────────────────────────────────────────────────────
DO $$
BEGIN
  -- Revoke from authenticated role (keep service_role for any admin backend usage)
  REVOKE EXECUTE ON FUNCTION public.place_order_with_wallet(
    TEXT, UUID, TEXT, DATE, DECIMAL, DECIMAL, DECIMAL, DECIMAL,
    JSONB, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DECIMAL
  ) FROM authenticated;
EXCEPTION WHEN OTHERS THEN
  -- Function may not exist in all environments — skip gracefully
  NULL;
END;
$$;

-- ────────────────────────────────────────────────────────────
-- 3. RESTRICT wallet credit to service_role only
--    Any authenticated user can currently call add_wallet_credit.
--    Remove that access — only service_role (backend) may credit wallets.
-- ────────────────────────────────────────────────────────────
REVOKE EXECUTE ON FUNCTION public.add_wallet_credit(UUID, DECIMAL, TEXT, TEXT, TEXT) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.add_wallet_credit(UUID, DECIMAL, TEXT, TEXT, TEXT) TO service_role;

-- Ensure order cancellation (which credits wallet) still works for authenticated
-- (the function body already verifies ownership/admin status)
GRANT EXECUTE ON FUNCTION public.handle_order_cancellation(TEXT, UUID, TEXT) TO authenticated, service_role;

-- ────────────────────────────────────────────────────────────
-- 4. RLS: Restrict admin_notifications INSERT to service_role only
--    Prevents any frontend user from spamming the admin panel.
-- ────────────────────────────────────────────────────────────
ALTER TABLE public.admin_notifications ENABLE ROW LEVEL SECURITY;

-- Drop existing permissive INSERT policy if it exists
DROP POLICY IF EXISTS "Authenticated can insert admin notifications" ON public.admin_notifications;
DROP POLICY IF EXISTS "Allow insert admin notifications" ON public.admin_notifications;
DROP POLICY IF EXISTS "Service role can insert admin notifications" ON public.admin_notifications;

-- Only service_role (and SECURITY DEFINER functions) and active admins can insert notifications
CREATE POLICY "Service role and admins can insert admin notifications"
