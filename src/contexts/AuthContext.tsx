...
interface AuthContextType {
  ...
  demoBypassLogin: (email: string) => void;
  isLoading: boolean;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
      ...
      const finalMessage =
        lastError?.message === 'Login server is responding slowly. Retrying...'
          ...
          : lastError?.message?.toLowerCase().includes('rate_limit') ||
            ...
            : lastError?.message?.toLowerCase().includes('invalid login credentials')
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          ...
          if (error) {
            ...
            const friendlyError = msg.includes('already registered')
  ...
  const updateProfile = async (data: Partial<User>) => {
    if (user) {
      ...
      try {
        const updatePayload: Partial<{ display_name: string }> = {};
  ...
  const demoBypassLogin = (email: string) => {
    setUser({
      id: 'demo-admin-id',
      email: email,
      name: email.split('@')[0],
  ...
  const contextValue = useMemo(() => ({
    ...
    user, supabaseUser, session, isInitializing, isLoading,
    login, signup, signInWithGoogle, resetPassword, logout,
    updateProfile, addOrder, addAddress, removeAddress, updateAddress, demoBypassLogin
  ]);
