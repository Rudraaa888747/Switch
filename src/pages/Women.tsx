...
const Women = () => {
  ...
  const colors = ['Black', 'White', 'Navy', 'Grey', 'Pink', 'Lavender', 'Floral'];
  const sizes = ['XS', 'S', 'M', 'L', 'XL', 'XXL'];
  const occasions = ['Casual', 'Formal', 'Party', 'Office', 'Wedding', 'Beach'];

  const { data: dbProducts = [], isLoading, isError, refetch } = useProducts({ category: 'women', limit: 24 });
  ...
  return (
    <>
      {/* Hero Section */}
      <section className="relative h-[60dvh] md:h-[70dvh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            <div className="bg-black/30 backdrop-blur-sm px-8 py-10 md:px-16 md:py-14 rounded-sm">
              ...
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-light tracking-[0.2em] text-white mb-6 drop-shadow-lg">
                WOMEN'S COLLECTION
              </h1>
              <p className="text-base md:text-lg text-white/90 font-light max-w-2xl mx-auto mb-8">
                Discover Smart Style — AI-Enhanced Fashion for Women. Get personalized suggestions, 
                style matches, and outfit ideas curated just for you.
              </p>
          ...
          <div className="flex-1">
            ...
            {isLoading && dbProducts.length === 0 ? (
              ...
              <div className="text-center py-20">
                <h3 className="text-xl font-medium mb-2 text-foreground">Unable to load products</h3>
                <p className="text-muted-foreground mb-6">Please check your connection and try again.</p>
                <button type="button" onClick={() => void refetch()} className="px-8 py-3 bg-foreground text-background text-sm tracking-wider hover:bg-foreground/90 transition-colors">TRY AGAIN</button>
              </div>
          ...
          >
            <div className="p-6">
              ...
              <div className="mt-8 pt-6 border-t border-border">
                <button
                  onClick={() => setShowFilters(false)}
                  className="w-full py-3 bg-foreground text-background text-sm tracking-wider"
                >
                  APPLY FILTERS
                </button>
              </div>
