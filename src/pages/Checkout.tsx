...
import { ChevronLeft, MapPin, CreditCard, Check, Shield, Package, ArrowRight, CheckCircle2, Wallet, Truck, Info } from 'lucide-react';
...
const OrderSummary = ({
  ...
  return (
  <div className="mobile-glass-panel rounded-[2rem] p-5 md:p-6">
    <div className="mb-5">
    ...
    <div className="mb-5 space-y-3">
      {items.map((item) => (
        <div key={`${item.product.id}-${item.size}-${item.color}`} className="flex gap-3 rounded-2xl border border-border/70 bg-card/70 p-3">
    ...
    <div className="space-y-3 text-sm">
      ...
      <div className="flex justify-between">
        <span className="text-muted-foreground flex items-center gap-1.5">
          {taxLabel}
          <Tooltip>
            <TooltipTrigger type="button" className="cursor-help text-muted-foreground/70 hover:text-foreground transition-colors" onClick={(e) => e.preventDefault()}>
              <Info size={14} />
            </TooltipTrigger>
            <TooltipContent className="max-w-[240px] text-xs p-3" side="top">
              <p className="font-semibold mb-1">Indian Apparel GST Slabs</p>
              <p className="text-muted-foreground mb-1">Calculated per individual item based on its unit price:</p>
              <ul className="list-disc pl-4 space-y-0.5 text-muted-foreground">
                <li><span className="text-foreground">5% GST</span> on items ≤ ₹1,000</li>
                <li><span className="text-foreground">12% GST</span> on items &gt; ₹1,000</li>
              </ul>
            </TooltipContent>
          </Tooltip>
        </span>
      ...
      <div className="border-t border-border pt-3">
        <div className="flex items-center justify-between text-lg font-semibold">
          ...
          <div className="flex flex-col items-end">
            {(couponDiscount > 0 || walletApplied > 0) && <span className="text-sm text-muted-foreground line-through font-normal">{formatPrice(actualGrandTotal)}</span>}
            <span className="text-emerald-500 font-bold">{formatPrice(Math.max(actualGrandTotal - walletApplied, 0))}</span>
          </div>
        </div>
      </div>
    </div>
...
const Checkout = () => {
  useDocumentTitle('Checkout | SWITCH');
  const navigate = useNavigate();
  const { items, totalPrice, clearCart, closeDrawer } = useCart();
  ...
  const [idempotencyKey, setIdempotencyKey] = useState<string | null>(() => sessionStorage.getItem('checkout_idempotency_key'));
  ...
  useEffect(() => {
    if (isAuthReady && !isAuthenticated) {
      ...
      if (addressForm.fullName || addressForm.address || addressForm.email) {
        sessionStorage.setItem('checkout_address', JSON.stringify(addressForm));
      }
      setCurrentStep('address');
      closeDrawer();
      navigate('/auth?returnTo=/checkout', { replace: true });
    }
  }, [isAuthReady, isAuthenticated, navigate, addressForm, closeDrawer]);
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    sessionStorage.setItem('checkout_idempotency_key', attemptKey);
    setIsProcessing(true);

    try {
      // Refresh Auth Session if logged in
      if (currentUserId) {
      ...
      const rpcArgs: Record<string, unknown> = {
        ...
        p_use_wallet: useWallet && !!currentUserId,
      };
      ...
      const res = await withTimeout(
        supabase.rpc('place_order_idempotent', { ...rpcArgs, p_idempotency_key: attemptKey }),
      ...
      const finalGrandTotal = parsed.total;
      ...
      // Mask raw database errors from being shown to the user
      const lowerError = errorMessage.toLowerCase();
      if (
        lowerError.includes('violates') || 
        lowerError.includes('null value in column') ||
        lowerError.includes('constraint') ||
        lowerError.includes('relation') ||
        lowerError.includes('duplicate key')
      ) {
        errorMessage = 'Something went wrong while placing your order. Please try again or contact support.';
      }
      
      if (errorMessage === 'Order placement timed out') {
        const { data: pendingResult, error: pendingError } = await supabase.rpc('get_checkout_idempotency_result', {
          p_idempotency_key: attemptKey,
        });
        const resolved = typeof pendingResult === 'string' ? JSON.parse(pendingResult) : pendingResult;
        if (!pendingError && resolved?.success && resolved?.order_id) {
          setOrderId(resolved.order_id);
          setConfirmedOrder({ id: resolved.order_id, total: resolved.total, discount: couponDiscount, couponCode: appliedCoupon?.code });
          clearCart();
          sessionStorage.removeItem('checkout_idempotency_key');
          setIdempotencyKey(null);
          setCurrentStep('confirmation');
  ...
  return (
    <>
      <div className="container-custom py-6 pb-[calc(var(--mobile-content-bottom)+6rem)] md:pt-[120px] md:pb-12">
        {currentStep !== 'confirmation' && (
          <button onClick={() => (currentStep === 'address' ? navigate('/cart') : setCurrentStep('address'))} className="group mb-6 flex w-fit items-center gap-2 text-sm text-muted-foreground transition-all hover:text-foreground">
                ...
                return (
                  <React.Fragment key={step.id}>
                    <div className={`flex flex-1 items-center justify-center gap-1.5 rounded-full px-2 py-2 text-[9.5px] uppercase tracking-[0.1em] whitespace-nowrap transition-colors ${
                      isActive ? 'bg-foreground text-background shadow-md' : isPast ? 'bg-emerald-500/10 text-emerald-500' : 'bg-muted text-muted-foreground'
                    }`}>
                      {isPast ? <CheckCircle2 size={14} className="shrink-0 text-emerald-500" /> : <Icon size={14} className="shrink-0" />}
                      <span className="truncate font-medium">{step.label}</span>
                    </div>
                    {index < steps.length - 1 && <div className={`h-px w-2 shrink-0 ${isPast ? 'bg-emerald-500' : 'bg-border'}`} />}
                  </React.Fragment>
                );
              })}
            </div>
        ...
        <AnimatePresence mode="wait">
          {currentStep === 'address' && (
            <motion.div key="address" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} className="grid gap-6 lg:grid-cols-3 lg:gap-8">
              <div className="lg:col-span-2">
                <div className="rounded-[2rem] border border-border/70 bg-card/80 p-5 shadow-[0_20px_50px_-40px_rgba(0,0,0,0.45)] md:p-8">
                  <h2 className="mb-6 text-xl font-semibold">Delivery Address</h2>

                  <form ref={addressFormRef} onSubmit={handleSubmit(handleAddressSubmit)} className="space-y-6">
                    <div className="grid gap-6 md:grid-cols-2">
                      <div className="relative">
                        <label className="mb-2 block text-sm font-medium">Full Name *</label>
                        <input type="text" {...register('fullName')} autoComplete="name" className={`input-premium transition-all duration-300 w-full ${errors.fullName ? 'border-rose-900/60 bg-rose-950/20 focus:border-rose-500/50' : ''}`} placeholder="Enter your full name" />
                        <AnimatePresence>
                          {errors.fullName && (
                            <motion.div initial={{ opacity: 0, y: -2 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -2 }} className="absolute -bottom-[18px] left-1 flex items-center gap-1.5">
                              <span className="h-1 w-1 rounded-full bg-rose-500 shadow-[0_0_6px_rgba(244,63,94,0.8)]" />
                              <p className="text-[10px] tracking-wide text-rose-400/90">{errors.fullName.message}</p>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </div>
                      <div className="relative">
                        ...
                        <input type="tel" inputMode="numeric" maxLength={10} {...register('phone', { onChange: (e) => { e.target.value = e.target.value.replace(/\D/g, '').slice(0, 10); } })} autoComplete="tel" className={`input-premium transition-all duration-300 w-full ${errors.phone ? 'border-rose-900/60 bg-rose-950/20 focus:border-rose-500/50' : ''}`} placeholder="10-digit mobile number" />
                        <AnimatePresence>
                          {errors.phone && (
                            <motion.div initial={{ opacity: 0, y: -2 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -2 }} className="absolute -bottom-[18px] left-1 flex items-center gap-1.5">
                              <span className="h-1 w-1 rounded-full bg-rose-500" />
                              <p className="text-[10px] tracking-wide text-rose-400/90">{errors.phone.message}</p>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" inputMode="email" {...register('email')} autoComplete="email" className={`input-premium transition-all duration-300 w-full ${errors.email ? 'border-rose-900/60 bg-rose-950/20 focus:border-rose-500/50' : ''}`} placeholder="Enter your email (optional)" />
                      <AnimatePresence>
                        {errors.email && (
                          <motion.div initial={{ opacity: 0, y: -2 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -2 }} className="absolute -bottom-[18px] left-1 flex items-center gap-1.5">
                            <span className="h-1 w-1 rounded-full bg-rose-500" />
                            <p className="text-[10px] tracking-wide text-rose-400/90">{errors.email.message}</p>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      <textarea {...register('address')} autoComplete="street-address" className={`input-premium min-h-[110px] resize-none transition-all duration-300 w-full ${errors.address ? 'border-rose-900/60 bg-rose-950/20 focus:border-rose-500/50' : ''}`} placeholder="House no, Building, Street, Area" />
                      <AnimatePresence>
                        {errors.address && (
                          <motion.div initial={{ opacity: 0, y: -2 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -2 }} className="absolute -bottom-[18px] left-1 flex items-center gap-1.5">
                            <span className="h-1 w-1 rounded-full bg-rose-500" />
                            <p className="text-[10px] tracking-wide text-rose-400/90">{errors.address.message}</p>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </div>
                    ...
                    <div className="grid gap-6 md:grid-cols-3">
                      <div className="relative">
                        ...
                        <input type="text" {...register('city')} autoComplete="address-level2" className={`input-premium transition-all duration-300 w-full ${errors.city ? 'border-rose-900/60 bg-rose-950/20 focus:border-rose-500/50' : ''}`} placeholder="City" />
                        <AnimatePresence>
                          {errors.city && (
                            <motion.div initial={{ opacity: 0, y: -2 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -2 }} className="absolute -bottom-[18px] left-1 flex items-center gap-1.5">
                              <span className="h-1 w-1 rounded-full bg-rose-500" />
                              <p className="text-[10px] tracking-wide text-rose-400/90">{errors.city.message}</p>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </div>
                      <div className="relative">
                        ...
                        <input type="text" {...register('state')} autoComplete="address-level1" className={`input-premium transition-all duration-300 w-full ${errors.state ? 'border-rose-900/60 bg-rose-950/20 focus:border-rose-500/50' : ''}`} placeholder="State" />
                        <AnimatePresence>
                          {errors.state && (
                            <motion.div initial={{ opacity: 0, y: -2 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -2 }} className="absolute -bottom-[18px] left-1 flex items-center gap-1.5">
                              <span className="h-1 w-1 rounded-full bg-rose-500" />
                              <p className="text-[10px] tracking-wide text-rose-400/90">{errors.state.message}</p>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </div>
                      <div className="relative">
                        ...
                        <input type="text" inputMode="numeric" pattern="[0-9]*" maxLength={6} {...register('pincode', { onChange: (e) => { e.target.value = e.target.value.replace(/\D/g, '').slice(0, 6); } })} autoComplete="postal-code" className={`input-premium transition-all duration-300 w-full ${errors.pincode ? 'border-rose-900/60 bg-rose-950/20 focus:border-rose-500/50' : ''}`} placeholder="6-digit pincode" />
                        <AnimatePresence>
                          {errors.pincode && (
                            <motion.div initial={{ opacity: 0, y: -2 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -2 }} className="absolute -bottom-[18px] left-1 flex items-center gap-1.5">
                              <span className="h-1 w-1 rounded-full bg-rose-500" />
                              <p className="text-[10px] tracking-wide text-rose-400/90">{errors.pincode.message}</p>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </div>
          ...
          {currentStep === 'transition' && (
            <motion.div key="transition" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4 }} className="grid gap-6 lg:grid-cols-3 lg:gap-8">
              <div className="lg:col-span-2">
                <div className="rounded-[2rem] border border-border/70 bg-card/60 p-5 md:p-8">
                  ...
                  <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="flex h-20 items-center gap-4 rounded-3xl border border-border/40 bg-background/40 px-4">
              ...
              <div className="lg:col-span-1 hidden lg:block">
                <div className="h-[400px] w-full animate-pulse rounded-[2rem] border border-border/50 bg-card/60" />
              </div>
            </motion.div>
          )}
              ...
              <div className="lg:col-span-2">
                <div className="rounded-[2rem] border border-border/70 bg-card/80 p-5 shadow-[0_20px_50px_-40px_rgba(0,0,0,0.45)] md:p-8">
                  ...
                  <div className="mb-6 space-y-3">
                    <div className={`overflow-hidden rounded-3xl border transition-all ${useWallet ? 'border-foreground bg-foreground/[0.04] shadow-[0_20px_45px_-36px_rgba(0,0,0,0.55)]' : 'border-border bg-card'}`}>
                    ...
                    {paymentMethods.map((method) => {
                      ...
                      return (
                        <div key={method.id} className={`overflow-hidden rounded-3xl border transition-all ${isSelected ? 'border-foreground bg-foreground/[0.04] shadow-[0_20px_45px_-36px_rgba(0,0,0,0.55)]' : 'border-border bg-card'}`}>
          ...
          {currentStep === 'confirmation' && (
            <motion.div key="confirmation" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="mx-auto max-w-2xl py-12 text-center">
              ...
              <div className="mb-8 rounded-[2rem] border border-border/70 bg-card/90 p-6 text-left">
    ...
    {currentStep !== 'confirmation' && currentStep !== 'transition' && !isKeyboardOpen && (
        <div className="sticky bottom-0 z-50 md:hidden bg-background/50 backdrop-blur-md border-t border-border/50 pb-4 pt-2 px-3">
          <div className="mobile-glass-panel rounded-[2rem] px-4 py-3 safe-bottom shadow-lg">
