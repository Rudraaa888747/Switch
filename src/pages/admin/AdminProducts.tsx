...
const AdminProducts = () => {
  const { isDemoMode, blockAction, simulateAction } = useDemoMode();
  ...
  const stats = useMemo(() => {
    ...
    return [
      { label: 'Total', value: total, icon: Package, color: 'text-white', iconGradient: 'from-indigo-400 to-indigo-600', haloColor: 'bg-indigo-500' },
      { label: 'Low Stock', value: lowStock, icon: AlertCircle, color: 'text-white', iconGradient: 'from-amber-400 to-amber-600', haloColor: 'bg-amber-500' },
      { label: 'Out of Stock', value: outOfStock, icon: X, color: 'text-white', iconGradient: 'from-rose-400 to-rose-600', haloColor: 'bg-rose-500' },
      { label: 'Trending', value: trending, icon: TrendingUp, color: 'text-white', iconGradient: 'from-emerald-400 to-emerald-600', haloColor: 'bg-emerald-500' },
    ];
  }, [dbProducts]);
  ...
  const addMutation = useMutation({
    mutationFn: async (newProduct: ProductFormValues) => {
      if (isDemoMode) {
        // Return a simulated product that mimics the database response
        return { ...newProduct, id: `demo-prod-${Date.now()}` } as ProductData;
      }
    ...
    onSuccess: () => {
      ...
      const successTitle = isDemoMode ? '✓ Product Simulated' : '✓ Product Added Successfully';
      toast({ title: successTitle, description: isDemoMode ? 'Demo Mode: Product added to local state only.' : undefined });
      setShowModal(false);
      resetForm();
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<ProductFormValues> }) => {
      if (isDemoMode) {
        return { id, ...updates } as ProductData;
      }
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin-overview'] });
      toast({ 
        title: isDemoMode ? '✓ Product Updated (Simulated)' : '✓ Product Updated Successfully',
        description: isDemoMode ? 'Demo Mode: Changes are visible locally but not saved.' : undefined
      });
      setShowModal(false);
      resetForm();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      if (isDemoMode) {
        throw new Error('DEMO_MODE_BLOCK');
      }
      const params = new URLSearchParams({ id: `eq.${id}` });
    ...
    onError: (error: Error) => {
      if (error.message === 'DEMO_MODE_BLOCK') {
        blockAction('deleteProduct');
        return;
      }
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    const cleanData: Record<string, unknown> = {
      ...(isEditing && selectedProduct ? {} : { id: `product-${Date.now()}` }),
      name: formData.name,
    ...
    if (isEditing && selectedProduct) {
      if (isDemoMode) {
        simulateAction('updateProduct', 600, () => {
      ...
      if (isDemoMode) {
        if (blockAction('addProduct')) return;
      } else {
  ...
  const handleVariantImageUpload = async (variantId: string, file: File) => {
    ...
    if (isDemoMode) {
      toast({ title: 'Simulating image upload...' });
      simulateAction('uploadImage', 800, () => {
        const fakeUrl = URL.createObjectURL(file);
        setVariants(prev =>
          prev.map(v =>
            v.id === variantId ? { ...v, images: [...v.images, fakeUrl] } : v
          )
        );
      }, 'Image uploaded to local preview');
      return;
    }
  ...
  return (
    <AdminLayout>
      <div className={isMobile ? 'px-3 py-3 space-y-3' : 'px-3 py-4 md:p-6 lg:p-8 space-y-6 md:space-y-8'}>
        {/* ── MOBILE HEADER ── */}
        {isMobile ? (
          <>
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-muted-foreground/70 tracking-wide">{dbProducts.length} items in catalog</p>
              </div>
              <button
                onClick={() => { resetForm(); setShowModal(true); }}
                className="flex items-center gap-1.5 bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-4 py-2.5 rounded-xl text-xs font-semibold shadow-lg shadow-indigo-500/25 active:scale-95 transition-transform shrink-0"
              >
                <Plus size={15} />
                Add
              </button>
            ...
            {/* Mobile stat pills — horizontal scroll */}
            <motion.div variants={containerVariants} initial="hidden" animate="show" className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide -mx-3 px-3">
              {stats.map((stat) => (
                <motion.div variants={itemVariants} key={stat.label} className={`flex items-center gap-2 px-3 py-2 rounded-xl border border-border/60 bg-card shrink-0`}>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-muted/60 border border-border/60 h-10 pl-9 pr-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/30 transition-all"
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5"><X size={14} className="text-muted-foreground" /></button>
                )}
              </div>
              <button
                onClick={() => setShowMobileFilters(v => !v)}
                className={`flex items-center justify-center w-10 h-10 rounded-xl border transition-all shrink-0 ${
                  categoryFilter !== 'all' ? 'bg-indigo-500/10 border-indigo-500/30 text-indigo-500' : 'bg-muted/60 border-border/60 text-muted-foreground'
                }`}
              >
                <Filter size={16} />
              </button>
        ...
        >
          <div>
            <h1 className="text-xl md:text-3xl font-bold mb-1 bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">Products</h1>
            <p className="text-xs md:text-base text-muted-foreground font-light">Manage your product catalog and inventory.</p>
          </div>
          ...
          <button
            onClick={() => {
              if (isDemoMode) {
                toast({
                  ...
                  description: 'Adding new products is restricted in Demo Mode.',
                  variant: 'destructive'
                });
                return;
              }
            ...
            className="flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white px-5 py-2.5 md:px-6 md:py-3 rounded-xl font-medium hover:from-indigo-600 hover:to-purple-700 transition-all shadow-lg shadow-indigo-500/25 active:scale-95 w-full sm:w-auto text-sm md:text-base"
          >
            <Plus size={20} />
            Add New Product
          </button>
          ...
          {stats.map((stat) => (
            ...
            >
              <div className="relative">
                ...
                <div className={`relative flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${stat.iconGradient} text-white shadow-xl shadow-black/10 ring-1 ring-inset ring-white/30`}>
        ...
        <div className={isMobile ? 'space-y-2' : 'space-y-4'}>
          {queryErrorMessage && (
            <div className="bg-rose-500/10 border border-rose-500/20 px-3 py-2.5 rounded-xl text-xs text-rose-500 flex items-center gap-2">
              <AlertCircle size={15} />
              {queryErrorMessage}
            </div>
          )}
          ...
          {isLoading ? (
            <div className="flex h-48 flex-col items-center justify-center gap-3">
              <Loader2 className="w-7 h-7 animate-spin text-indigo-500/50" />
            ...
            <>
              {filteredProducts.length === 0 ? (
                ...
                <div ref={parentRef} className="h-[70dvh] overflow-y-auto custom-scrollbar overflow-x-hidden">
                  ...
                  {rowVirtualizerMobile.getVirtualItems().map((virtualRow) => {
                    ...
                    >
                      <div className="relative bg-card border border-border/60 rounded-2xl overflow-hidden active:scale-[0.99] transition-transform">
                      <div
                        className="flex items-center gap-3 p-3"
                        onClick={() => handleEditClick(product)}
                      >
                        {/* Product Image */}
                        <div className="w-14 h-[4.25rem] bg-muted rounded-xl overflow-hidden border border-border/40 shrink-0 group">
                          {product.image_url ? (
                            <img
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {product.original_price && product.original_price > product.price && (
                              <span className="text-[10px] text-muted-foreground/60 line-through">{formatPrice(product.original_price)}</span>
                            )}
                          </div>
                          
                          <details className="mt-1.5 group/details" onClick={(e) => e.stopPropagation()}>
                            <summary className="text-[10px] text-indigo-500 font-medium cursor-pointer list-none flex items-center gap-1">
                            ...
                            <div className="flex flex-wrap items-center gap-2 mt-2 pb-1">
                              <span className="px-1.5 py-0.5 bg-muted/80 text-[9px] font-semibold uppercase tracking-wider text-muted-foreground rounded">{product.category}</span>
                              <div className="flex items-center gap-1">
                                <div className={`w-1.5 h-1.5 rounded-full ${
                                  product.stock_quantity === 0 ? 'bg-rose-500' :
                                  product.stock_quantity <= 10 ? 'bg-amber-500' : 'bg-emerald-500'
                                }`} />
                                <span className="text-[10px] text-muted-foreground">{product.stock_quantity}</span>
                              </div>
                              {product.is_new && (
                                <span className="text-[8px] px-1.5 py-0.5 bg-indigo-500/10 text-indigo-500 rounded-full font-bold uppercase">New</span>
                              )}
                              {product.is_trending && (
                                <span className="text-[8px] px-1.5 py-0.5 bg-emerald-500/10 text-emerald-500 rounded-full font-bold uppercase">Hot</span>
                              )}
                            </div>
                          </details>
                        </div>
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setMobileActionMenu(mobileActionMenu === product.id ? null : product.id);
                          }}
                          className="p-2 rounded-xl text-muted-foreground/50 shrink-0"
                        >
                        ...
                        {mobileActionMenu === product.id && (
                          ...
                          >
                            <div className="flex items-center gap-2 px-3 py-2.5 bg-muted/30">
                              <button
                                onClick={() => { handleEditClick(product); setMobileActionMenu(null); }}
                                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-indigo-500/10 text-indigo-500 text-xs font-semibold transition-all active:scale-95"
                              >
                                <Edit size={13} />
                                Edit
                              </button>
                              <button
                                ...
                                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-rose-500/10 text-rose-500 text-xs font-semibold transition-all active:scale-95"
                              >
                                <Trash2 size={13} />
                                Delete
                              </button>
              ...
              {filteredProducts.length > 0 && (
                <p className="text-center text-[10px] text-muted-foreground/50 pt-2 pb-4">
                  {filteredProducts.length} of {dbProducts.length} products
                </p>
              )}
            </>
          ) : (
            ...
            >
              <div className="overflow-x-auto">
                <div className="w-full text-left flex flex-col min-w-[800px]">
                  <div className="grid grid-cols-5 bg-gradient-to-r from-muted/80 to-muted/30 border-b border-border text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                    <div className="px-6 py-4">Product</div>
                  ...
                  <div ref={parentRefDesktop} className="h-[65vh] overflow-y-auto custom-scrollbar">
                  {filteredProducts.length === 0 ? (
                    <div className="px-6 py-12 text-center text-muted-foreground">
                      No products found matching your search.
                    </div>
                  ) : (
                    <div style={{ height: `${rowVirtualizerDesktop.getTotalSize()}px`, width: '100%', position: 'relative' }}>
                    {rowVirtualizerDesktop.getVirtualItems().map((virtualRow) => {
                      const product = filteredProducts[virtualRow.index];
                      return (
                        <div
                          key={product.id}
                          className="absolute top-0 left-0 w-full grid grid-cols-5 items-center transition-colors hover:bg-muted/30 border-b border-border/50"
                          style={{
        ...
        <AnimatePresence>
          {showModal && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }}
              className={`fixed inset-0 z-[100] flex items-end md:items-center justify-center ${isMobile ? 'bg-black/60' : 'bg-background/80 backdrop-blur-sm p-2 md:p-4'}`}
            ...
            >
              <motion.div
                ...
                className={`bg-card overflow-hidden flex flex-col ${isMobile ? 'w-full h-[95dvh] rounded-t-2xl border-t border-x border-border' : 'border border-border rounded-2xl w-full max-w-6xl shadow-2xl max-h-[95vh]'}`}
                onClick={(e) => e.stopPropagation()}
              >
                <div className={`border-b border-border flex items-center justify-between shrink-0 ${isMobile ? 'px-4 py-3' : 'px-6 py-4'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className={isMobile ? 'text-base font-bold' : 'text-xl font-bold'}>
                ...
                <div className={`flex-1 overflow-y-auto ${isMobile ? 'p-4' : 'p-6'}`}>
                  ...
                  <form id="product-form" onSubmit={handleSubmit} className="flex flex-col lg:flex-row gap-8">
                    <div className="flex-1 min-w-0 space-y-6">
                      <div className={`flex items-center gap-2 border-b border-border/50 pb-3 mb-3 overflow-x-auto custom-scrollbar ${isMobile ? '-mx-1 px-1' : ''}`}>
                        {(['general', 'inventory', 'variants', 'seo'] as const).map(tab => (
                          <button
                            key={tab}
                            type="button"
                            onClick={() => setActiveTab(tab)}
                            className={`px-3 py-1.5 text-xs font-semibold capitalize rounded-lg transition-all whitespace-nowrap ${activeTab === tab ? 'bg-indigo-500 text-white shadow-md shadow-indigo-500/20' : 'text-muted-foreground hover:bg-muted/50 hover:text-foreground'}`}
                          >
                            {tab}
                          </button>
                        ))}
                    ...
                    {/* Live Preview — hidden on mobile to save space */}
                    {!isMobile && (
                    <motion.div
                        ...
                        {previewImages.length > 1 && (
                          <div className="flex gap-2 overflow-x-auto pb-1">
                            {previewImages.map((img, idx) => (
                              ...
                              >
                                <img loading="lazy" decoding="async" src={normalizeImageUrl(img)} alt="" className="w-full h-full object-cover" />
                              </motion.button>
                            ))}
                          </div>
                        )}
                ...
                <div
                  className={`border-t border-border bg-gradient-to-r from-muted/50 to-muted/20 flex items-center gap-3 shrink-0 ${isMobile ? 'px-4 py-3 safe-bottom' : 'px-6 py-4 justify-end'}`}
                >
                  <button
                    type="button"
                    onClick={() => setShowModal(false)}
                    className={`text-sm font-medium text-muted-foreground hover:text-foreground transition-colors ${isMobile ? 'flex-1 py-2.5 text-center rounded-xl border border-border' : 'px-4 py-2'}`}
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl text-sm font-medium hover:from-indigo-600 hover:to-purple-700 transition-all disabled:opacity-50 shadow-lg shadow-indigo-500/20 ${isMobile ? 'flex-1 py-2.5' : 'px-6 py-2.5'}`}
                  >
