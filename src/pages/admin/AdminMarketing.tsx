...
const saveData = async (data: MarketingData) => {
...
const AdminMarketing = () => {
  ...
  const { isDemoMode, simulateAction, blockAction } = useDemoMode();
  ...
  useEffect(() => {
    if (loaded) {
      void saveData({ campaigns, coupons, newsletterDraft }).catch((error) => {
        console.error('Failed to save marketing content:', error);
        toast({ title: 'Marketing content not saved', description: 'Please try again.', variant: 'destructive' });
      });
    }
  ...
  const handleAddCampaign = () => {
    ...
    if (isDemoMode) {
      simulateAction('addCampaign', 600, () => {
        persistCampaigns([campaign, ...campaigns]);
        setShowCampaignModal(false);
        setNewCampaign({ name: '', type: 'email', status: 'draft', audience: '', scheduledDate: '' });
        toast({ title: 'Campaign Created (Simulated)', description: `Demo Mode: ${campaign.name} created locally.` });
      });
      return;
    }
  ...
  const handleSaveCoupon = () => {
    ...
    if (editingCoupon) {
      ...
      if (isDemoMode) {
        simulateAction('updateCoupon', 500, () => {
          persistCoupons(next);
          toast({ title: 'Coupon Updated (Simulated)', description: `Demo Mode: Coupon ${couponForm.code} updated locally.` });
      ...
      if (isDemoMode) {
        simulateAction('addCoupon', 500, () => {
          persistCoupons([...coupons, coupon]);
          toast({ title: 'Coupon Added (Simulated)', description: `Demo Mode: Coupon ${couponForm.code} created locally.` });
          setShowCouponModal(false);
          setEditingCoupon(null);
        });
        return;
  ...
  const handleSendNewsletter = () => {
    ...
    if (isDemoMode) {
      simulateAction('sendNewsletter', 800, () => {
        toast({ title: 'Newsletter Simulated', description: `Demo Mode: "${newsletterDraft.subject}" was pseudo-sent to all subscribers.` });
      });
      return;
    }
        ...
        {campaigns.map((campaign, i) => {
          ...
          return (
            <motion.div
              ...
              className="admin-glass-card rounded-3xl p-5 hover:shadow-md hover:border-primary/20 transition-all"
            >
