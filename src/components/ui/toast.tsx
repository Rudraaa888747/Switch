...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-2xl border p-4 pr-6 shadow-2xl transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border-white/10 bg-background/80 backdrop-blur-xl text-foreground shadow-[0_8px_30px_rgb(0,0,0,0.4)]",
        destructive: "destructive group border-rose-900/40 bg-background/90 backdrop-blur-xl text-foreground shadow-[0_8px_30px_rgb(0,0,0,0.5),inset_0_0_20px_rgba(244,63,94,0.05)]",
      },
    },
    defaultVariants: {
      variant: "default",
