...
// Mobile-specific flat nav structure — cleaner hierarchy for phones
const mobileCorNav = [
  { icon: LayoutDashboard, label: 'Dashboard', path: '/admin/dashboard' },
  { icon: Package, label: 'Products', path: '/admin/products', permission: 'manage_products' },
  { icon: ShoppingCart, label: 'Orders', path: '/admin/orders', permission: 'manage_orders' },
  { icon: Undo2, label: 'Returns', path: '/admin/returns' },
  { icon: Users, label: 'Customers', path: '/admin/users', permission: 'manage_users' },
  { icon: MessageSquare, label: 'Reviews', path: '/admin/reviews' },
];
...
const mobileSecondaryNav = [
  { icon: BarChart3, label: 'Inventory', path: '/admin/inventory' },
  { icon: TrendingUp, label: 'Analytics', path: '/admin/analytics' },
  { icon: Megaphone, label: 'Marketing', path: '/admin/marketing', permission: 'manage_marketing' },
  { icon: FileBarChart, label: 'Reports', path: '/admin/reports' },
  { icon: UserCog, label: 'Staff', path: '/admin/staff', permission: 'manage_staff' },
];
...
const AdminSidebar = ({ onMobileClose }: AdminSidebarProps) => {
  ...
  if (isMobile) {
    return (
      <aside className="flex h-screen w-[min(85vw,22rem)] flex-col bg-background safe-top safe-bottom">
        {/* Profile Header */}
        <div className="flex items-center justify-between border-b border-border/50 px-5 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold">{adminName || 'Admin'}</p>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide px-3 py-4">
            ...
            {mobileCorNav.map((item) => {
              ...
              return (
                <Link
                  ...
                  to={item.path}
                  onClick={onMobileClose}
                  className={`flex items-center gap-3.5 rounded-2xl px-4 py-3.5 text-sm font-medium transition-all duration-200
                    ${active
                      ? 'bg-foreground text-background'
                      : 'text-foreground/80 active:bg-muted/70'
                    }`}
                >
          ...
          <p className="mb-2 px-4 text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground/60">
            Insights & Tools
          </p>
          <div className="space-y-0.5">
            {mobileSecondaryNav.map((item) => {
              ...
              return (
                <Link
                  ...
                  to={item.path}
                  onClick={onMobileClose}
                  className={`flex items-center gap-3.5 rounded-2xl px-4 py-3 text-sm transition-all duration-200
                    ${active
        ...
        <div className="border-t border-border/50 px-3 py-3 space-y-1">
          <div className="flex items-center justify-between px-4 py-2 mb-1">
            <span className="text-[10px] uppercase tracking-[0.2em] font-semibold text-muted-foreground">Theme</span>
            <ThemeToggle />
          </div>
          <Link
            to="/admin/settings"
          ...
          <button
            ...
            className="flex w-full items-center gap-3.5 rounded-2xl px-4 py-3 text-sm text-destructive/80 transition-all duration-200 active:bg-destructive/10"
          >
            <LogOut size={18} />
            <span>Logout</span>
          </button>
  ...
  return (
    ...
    className={`h-screen flex-col border-r border-border bg-card/96 backdrop-blur-md sticky top-0 z-30 flex ${sidebarCollapsed ? 'w-16' : 'w-64'}`}
      layout
    >
                    ...
                    <div key={item.path}>
                      {hasSubmenu ? (
                        <button
                          ...
                          className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 transition-all ${active ? 'bg-foreground/[0.08] text-foreground' : 'text-muted-foreground hover:bg-muted/60 hover:text-foreground'}`}
                        ...
                        <Link
                          to={item.path}
                          className={`flex items-center gap-3 rounded-2xl px-3 py-3 transition-all ${active ? 'bg-foreground text-background shadow-[inset_0px_2px_5px_rgba(255,255,255,0.15),0_18px_38px_-28px_rgba(0,0,0,0.65)]' : 'text-muted-foreground hover:bg-muted/60 hover:text-foreground'}`}
                          title={sidebarCollapsed ? item.label : undefined}
                        >
                          <Icon size={19} />
                          <AnimatePresence mode="wait">
                              ...
                              {item.submenu!.map((sub) => {
                                ...
                                return (
                                  <Link
                                    key={sub.path}
                                    to={sub.path}
                                    className={`flex items-center gap-2.5 rounded-xl px-3 py-2 text-xs transition-all ${subActive ? 'bg-foreground/[0.08] text-foreground' : 'text-muted-foreground/78 hover:bg-muted/60 hover:text-foreground'}`}
                                  >
                                    <SubIcon size={14} />
                                    {sub.label}
                                  </Link>
      ...
      <div className="border-t border-border p-2">
        <AnimatePresence mode="wait">
          {!sidebarCollapsed ? (
            <motion.div key="expanded-footer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mb-1 rounded-2xl bg-muted/40 px-3 py-3">
        ...
        <div className="flex items-center gap-1 px-1">
          ...
          <button onClick={handleLogout} className="flex flex-1 items-center justify-center gap-2 rounded-xl px-3 py-2 text-xs text-muted-foreground transition-all hover:bg-destructive/10 hover:text-destructive">
            <LogOut size={16} />
            <AnimatePresence mode="wait">
              {!sidebarCollapsed && (
          ...
          <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="rounded-xl p-2 text-muted-foreground transition-all hover:bg-muted hover:text-foreground">
