...
export const supabaseRestRequest = async <T>(
  ...
  options?: {
    ...
    /** Public endpoints do not need to wait for Supabase Auth to initialize. */
    useSession?: boolean;
  }
) => {
  let token = options?.authToken;
  if (!token && options?.useSession !== false) {
    try {
  ...
  // Use AbortSignal to timeout after 20 seconds to allow for Supabase cold starts
...
export const supabaseRestSelect = async <T>(
  path: string,
  searchParams?: URLSearchParams,
  authToken?: string | null,
  useSession = true
